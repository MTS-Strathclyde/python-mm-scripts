...
import os
import sys
from cookbook import utility as fileutil
from comp_chem import gaussian

__version__ = 1.1


DEFAULT_KWRDS = """m062x/gen scf=tight scrf=(smd,solvent={}) pop=Hirshfeld"""
TS_KWRDS = """opt=(calcfc,tight,ts,noeigentest) freq wb97xd/gen/w06
scrf=(smd,solvent={0}) int=ultrafine"""
DEFAULT_BASIS = """/home/max/Documents/ProgramFiles/GaussianBasisSets/MG3S.json"""
...
def process_command_line(argv):
    ...
    parser.add_argument('-b', '--basis', help="""In case of custom basis
                        ...
                        [/home/max/Documents/ProgramFiles/GaussianBasisSets/MG3S.json]""",
                        default=DEFAULT_BASIS)
    parser.add_argument('-e', '--noECP', help="""Don't add to kwards pseudo=read""",
                        action='store_true')
    parser.add_argument('-s', '--solvent', help='Solvent name recognized by gaussian (water)',
                        metavar='<solvent>', default='water')      
    parser.add_argument('-d', '--dir', help="""Output dir. (.)""", default='.')
    parser.add_argument('-n', '--name', help="""Name modifier.""", default='')
    parser.add_argument('-c', '--chg', help=""" Molecule charge [0].""", default='0')
    parser.add_argument('-p', '--proc', help=""" Number of processors [4].""", 
                        default=4, type=int)
...
def handle_mol(mol_name, args):
    ...
    kwrds = args.kwards.format(args.solvent)
    if not args.noECP:
        kwrds += ' pseudo=read'
    if args.chg:
        std_input.set_charge(args.chg)
    std_input.set_kwards(kwrds)
    try:
        std_input.add_custom_basis_from_dic(args.basis, not args.noECP)
    except KeyError:
        print("Skipping mol {}".format(mol_name))
        return 1
