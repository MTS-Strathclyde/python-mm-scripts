...
REMOVE_KWARDS_STARTING_WITH = ['opt', 'gen', 'pseudo', '6-3']
ADD_KWARDS = ['freq=noraman', 'chkbas', 'guess=tcheck', 'geom=allcheck']
...
def change_kwards(kward_txt):
    ...
    for kw in old_kward_list:
        good_kw = True
        for rm_kw in REMOVE_KWARDS_STARTING_WITH:
            if kw.startswith(rm_kw):
                good_kw = False
        if good_kw:
